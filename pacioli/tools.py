...
class AccountTools:
    ...
    def import_csv(self, file):
        ...
        for record in inputdata:
            for key in ['name', 'credit', 'type']:  # Make sure all the required fields are there
                assert key in record.keys()
            clean = {'name': record['name'], 'type': record['type']} # Create cleaned set and intialise with the strings
            ...
            if record['credit'] == "FALSE":
                ...
                raise ValueError("{} is not a valid value for credit".format(record['credit']))

            # Check for the optional values
            # Handle parent if present
            if 'parent' in record.keys():
                if record['parent'] == '':
                    record['parent'] = None  # An empty value can't be dealt with, but None works
        ...
        for record in data:
            ...
            if "parent" in record.keys() and record['parent'] is not None:
                try:
                    ...
                    logging.warning("Parent {} could not be found for {}".format(record['parent'], record['name']))
            acc = Accounts(name=record['name'], credit=record['credit'], type=record['type'], parent=record['parent'])
            try:
                acc.full_clean()
